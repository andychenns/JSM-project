...
```{r Models}
#Rent only
model1 <- glm(Gentrified ~ Avg_Rent, data = avg_NY, family = "binomial")
summary(model1)
```
```{r}
#Income only
model2 <- glm(Gentrified ~ Avg_Income, data = avg_NY, family = "binomial")
summary(model2)
```
```{r}
#Rent & Income
model3 <- glm(Gentrified ~ Avg_Rent + Avg_Income, data = avg_NY, family = "binomial")
summary(model3)
```
```{r}
#Rent, Income & Interaction
model4 <- glm(Gentrified ~ Avg_Rent + Avg_Income + Avg_Rent*Avg_Income, data = avg_NY, family = "binomial")
summary(model4)
```
```{r}
model5 <- glm(Gentrified ~ factor(Borough) +  Avg_Rent + Avg_Income + Avg_Rent*Avg_Income + Avg_NPR, data = avg_NY, family = "binomial")
summary(model5)
```
