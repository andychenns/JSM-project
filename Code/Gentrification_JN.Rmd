...
pct_NY <- mutate(avg_NY, Row = 1:n()) %>%
  ...
  ungroup %>%
<<<<<<< HEAD
  select(Row, Year, Sub_Borough, Gentrified, Avg_Age, Pct_Chg_Income, Pct_Chg_Rent, Rent_Inc, Pct_White) %>%
  na.omit(pct_NY)
```

```{r Final Model}
#% change in Rent, Income, % White, Avg Age
model <- glm(Gentrified ~ Pct_Chg_Income + Pct_Chg_Rent + Pct_Chg_Income*Pct_Chg_Rent + Pct_White + Avg_Age, data = pct_NY, family = "binomial")
=======
  select(Row, Year, Sub_Borough, Gentrified, Avg_Age, Pct_Income, Pct_Rent, Rent_Inc, Pct_White) %>%
  na.omit(pct_NY)
```
...
```{r Final Model}
#% change in Rent, Income, % White, Avg Age
model <- glm(Gentrified ~ Pct_Income + Pct_Rent + Pct_White + Avg_Age, data = pct_NY, family = "binomial")
>>>>>>> fc999b8c588ea7274c44d7fb7caa928d9c15e62d
summary(model)
```
