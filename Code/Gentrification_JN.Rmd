...
pct_NY <- mutate(avg_NY, Row = 1:n()) %>%
  ...
  mutate(Pct_Income = Avg_Income/lag(Avg_Income) -1 ) %>%
  mutate(Pct_Rent = Avg_Rent/lag(Avg_Rent) -1 ) %>%
  ungroup %>%
  select(Row, Year, Sub_Borough, Gentrified, Avg_Age, Pct_Income, Pct_Rent, Rent_Inc) %>%
  na.omit(pct_NY)
```
...
```{r Models}
#% change in Rent only
model1 <- glm(Gentrified ~ Pct_Rent, data = pct_NY, family = "binomial")
summary(model1)
```

```{r}
#% change in Income only
model2 <- glm(Gentrified ~ Pct_Income, data = pct_NY, family = "binomial")
summary(model2)
```

```{r}
#% change in Rent & Income
model3 <- glm(Gentrified ~ Pct_Income + Pct_Rent + Rent_Inc , data = pct_NY, family = "binomial")
summary(model3)
```
