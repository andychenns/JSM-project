...
```{r}
wind_1991 <- gen_1 %>% filter((`year` == 91))
wind_1991_bronx <- gen_1 %>% filter((`borough` == 1))
wind_1991_brooklyn <- gen_1 %>% filter((`borough` == 2))
wind_1991_manhattan <- gen_1 %>% filter((`borough` == 3))
wind_1991_queens <- gen_1 %>% filter((`borough` == 4))
wind_1991_staten <- gen_1 %>% filter((`borough` == 5))


ggplot(wind_1991_bronx,aes(as.factor(wind_1991_bronx$sba),fill=as.factor(wind_1991_bronx$X_e3)))+
  geom_bar(position="fill") + labs(title = "Broarded Up Windows by Sub-Boroughs", subtitle = "Bronx 1991: First Generation Immigrants", x = "Sub-Borough", y = "Count") + scale_fill_discrete(name = "Conditions", labels = c("Boarded Up", "Not Reported", "Not Boarded")) 
#summary(wind_1991_bronx)
```
