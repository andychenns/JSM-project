...
ggplot(DF[DF$Gen != 0,])+
  aes(x = sba)+
  geom_bar(aes(fill = factor(Gen)), position = "fill")+
  labs(title = "1st and 2nd Gen Immigrants by Sub-Borough", y = "Proportion", x = "Sub-Borough")+
  scale_fill_brewer(name = "Generation", palette = "Accent")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, size = 7))
ggsave("Sub_Gen12.png")
```
...
ggsave("C:/Users/GP/Desktop/MEGHA/SemII/JSM-ASA Challenge/JSM-project/Code/Gen1_birth.png")
...
ggplot(DF[DF$Gen == 1,])+
  ...
  labs(title = "Household Composition by Borough- 1st Gen", x = "Composition Code")+
  facet_wrap(Bor ~.)+
...
ggplot(DF[DF$Gen == 2,])+
  aes(x = factor(as.numeric(hcr)))+
  geom_bar(aes(fill = factor(as.numeric(hcr))))+
  labs(title = "Household Composition by Borough - 2nd Gen", x = "Composition Code")+
  facet_wrap(Bor ~.)+
  scale_fill_discrete(labels = c("1" = "1 - Married Couple\nWith no other household members",
                                                "2" = "2 - Married Couple\nWith children under 18 only",
                                                "3" = "3 - Married Couple\nWith no children under 18",
